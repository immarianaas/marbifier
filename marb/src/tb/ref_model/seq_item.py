class SeqItem():
    ...
    def clone(self):
        cp = SeqItem(self.DATA_WIDTH, self.ADDR_WIDTH)
        cp.set_data(self.rd, self.wr, self.addr, self.wr_data)
        return cp
...
class SeqItemOut(SeqItem):
    ...
    def __str__(self):
        return f"{super().__str__()}, rd_data={self.rd_data}, ack={self.ack}, ADDR_WIDTH={self.ADDR_WIDTH}, DATA_WIDTH={self.DATA_WIDTH}"
